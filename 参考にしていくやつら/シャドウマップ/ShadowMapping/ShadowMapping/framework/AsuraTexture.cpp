...
//　link
//
#pragma comment(lib, "framework/lib/glew32.lib")
#if _DEBUG
#pragma comment(linker, "/nodefaultlib:libcmtd")
#pragma comment(lib, "framework/lib/libpngd.lib")
#pragma comment(lib, "framework/lib/zlibd.lib")
#pragma comment(lib, "framework/lib/libjpegd.lib")
#else
#pragma comment(linker, "/nodefaultlib:libc")
#pragma comment(linker, "/nodefaultlib:libcd")
#pragma comment(linker, "/nodefaultlib:libcmtd")
#pragma comment(lib, "framework/lib/libpng.lib")
#pragma comment(lib, "framework/lib/zlib.lib")
#pragma comment(lib, "framework/lib/libjpeg.lib")
#endif
#endif
...
//　　Desc : コンストラクタ
//--------------------------------------------------------------------------------------------------
AsuraTexture::AsuraTexture(GLenum target)
{
	strcpy(m_nameA, "none");
...
//　　Desc : パラメータの設定
//-------------------------------------------------------------------------------------------------
void AsuraTexture::Parameter(GLenum pname, GLint param)
{
...
//　　Desc : 環境の設定
//-------------------------------------------------------------------------------------------------
void AsuraTexture::Enviorement(GLenum pname, GLint param)
{
	glBindTexture(m_target, m_handle);
...
//　　operator =
//-------------------------------------------------------------------------------------------------
AsuraTexture &AsuraTexture::operator =(const AsuraTexture &texture)
{
...
//　　operator ==
//-------------------------------------------------------------------------------------------------
bool AsuraTexture::operator== (const AsuraTexture &texture)
{
...
//　　Desc : 空テクスチャを作成
//-------------------------------------------------------------------------------------------------
void AsuraCreateTexture(GLuint width, GLuint height, GLuint format, AsuraTexture &texture)
{
	GLuint tempHandle=0;
...
#if USE_GLEW
	case ASURA_DDS:
		wcout << L"Error : DDSには対応していません\n";
		result = false;
		break;
#endif

	default :
		wcout << L"Error : フォーマットが不正です\n";
		result = false;
		break;
	}
	return result;
...
bool AsuraSaveSurfaceToFileA(char *filename, AsuraRect rect, ASURA_IMAGE_FILEFORMAT format)
{
	...
	bool result = AsuraCreateImageFileA(filename, rect.right-rect.left, rect.bottom-rect.top, format, texel);
	if ( texel )
	{
		delete [] texel;
		texel = NULL;
...
bool AsuraSaveSurfaceToFileW(wchar_t *filename, AsuraRect rect, ASURA_IMAGE_FILEFORMAT format)
{
	...
	texel = new GLubyte [(rect.right-rect.left)*(rect.bottom-rect.top)*4];
	glReadPixels(rect.left, rect.top,  rect.right-rect.left, rect.bottom-rect.top, GL_RGBA, GL_UNSIGNED_BYTE, texel);
	bool result = AsuraCreateImageFileW(filename, rect.right-rect.left, rect.bottom-rect.top, format, texel);
	if ( texel )
	{
		delete [] texel;
		texel = NULL;
